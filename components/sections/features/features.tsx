...
const getIcon = (iconName: string) => {
  ...
  return icon ? <FontAwesomeIcon icon={icon} className="w-5 h-5 text-atcat-100"/> : null;
}

export const Features = () => {
    return (
        <div className="flex flex-col gap-4 mt-2 sm:mt-4">
             <h1 className="text-center uppercase text-sm sm:text-lg text-gray-500 sm:text-atcat-100/80 mt-10">
                {"Guided learning"}
            </h1>
            <div className="px-16 flex flex-col md:flex-row gap-4 mx-auto md:w-[70vw]">
               
                {gl_features1.map((card) => (
                    <Card key={card.id} className=" flex flex-col gap-1 w-full sm:hover:shadow-[0_0_30px_rgba(17,182,242,1)] sm:hover:scale-[102%]  transition-all duration-300 bg-gradient-to-br to-20% from-gray-700 to-60% to-black/90 rounded-lg p-2 border-[1px] border-atcat-100  sm:border-gray-300/40">
            ...
            <h1 className="flex flex-row items-center gap-2 text-center items-center justify-center uppercase text-sm sm:text-md  mt-10 mx-auto">
                <p>{getIcon("Star")}{getIcon("Star")}{getIcon("Star")}</p>
                <p >{"but"}</p>  
                <p>{getIcon("Star")}{getIcon("Star")}{getIcon("Star")}</p>
            </h1>
            ...
            <div className="px-16 flex flex-col md:flex-row gap-4 mx-auto md:w-[70vw]">
                ...
                {but_features2.map((card) => (
                    <Card key={card.id} className=" flex flex-col gap-1 w-full hover:shadow-[0_0_30px_rgba(17,182,242,1)] hover:scale-[102%]  transition-all duration-300 bg-gradient-to-br to-20% from-gray-700 to-60% to-black/90 rounded-lg p-2 border-[1px] border-atcat-100 sm:border-gray-300/40">
                        ...
                        <p className="uppercase text-center text-sm text-gray-200 mb-2">Don't worry, we'll let you know when you're ready to join.</p>
                    </Card>
                ))}
